...
class SearchPage extends Component<Props> {
  constructor(props) {
    ...
    this.isKeyChange = false;//if the search keyword which hasn't been added to popular tabs
  }
  ...
  onBackPress() {
    ...
    onSearchCancel();//cancel search when exit
    this.refs.input.blur();
    NavigationUtil.goBack(this.props.navigation);
    if (this.isKeyChange) {
      onLoadLanguage(FLAG_LANGUAGE.flag_key);//reload popular tabs
    }
    return true;
  }
  ...
  saveKey() {
    ...
    if (Utils.checkKeyIsExist(keys, key)) {
      this.toast.show(key + ' has been existed.');
    } else {
      key = {
        "path": key,
        "name": key,
      ...
      keys.unshift(key);//add tab to the front of tabs
      this.languageDao.save(keys);
      this.toast.show(key.name + ' has been saved.');
  ...
  analyticSearchKey() {
    console.log(`tracking event: search keywords (${this.inputKey})`);
    Analytics.record({
      name: 'Search keywords',
      attributes: {keywords: this.inputKey}
    });
  ...
  renderNavBar() {
    ...
    const placeholder = inputKey || "Please input...";
    let backButton = ViewUtil.getLeftBackButton(() => this.onBackPress());
    let inputView = <TextInput
      ref="input"
      placeholder={placeholder}
      onChangeText={text => this.inputKey = text.trim()}
    ...
    let rightButton =
      <TouchableOpacity
        onPress={() => {
          this.refs.input.blur();//hide keyboard
  ...
  render() {
    ...
    let listView = !isLoading ? <FlatList
      ...
      onEndReached={() => {
        ...
        setTimeout(() => {
          if (this.canLoadMore) {//fix when scroll  call onEndReached twice
            this.loadData(true);
      ...
      onMomentumScrollBegin={() => {
        this.canLoadMore = true; //fix initialize page call onEndReached
    ...
    let bottomButton = showBottomButton ?
      ...
      >
        <View style={{justifyContent: 'center'}}>
          <Text style={styles.title}>Add to popular tabs</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  tabStyle: {
    // minWidth: 50 //fix minWidth cause tabStyle flash when first load会
    padding: 0
  },
  indicatorStyle: {
    height: 2,
  ...
  bottomButton: {
    ...
    position: 'absolute',
    left: 10,
    top: GlobalStyles.window_height - 45 - (DeviceInfo.isIPhoneX_deprecated ? 34 : 25),
    right: 10,
    borderRadius: 3
  },
  centering: {
